...
$\mathbb{F}_2$ polynomial that defines it, its expression $\wexpr{w_i}$ read off
...
Definition~\ref{def:wire-function} are what the soundness proofs appeal to. It
...
% Again, here we need to precisely define what T is. From Line 19, I think it is a stack of pairs. Also, an intuitive meaning of a derivation must be given here.
...
% refer to the soundness proof
  ...
  right way, it hands back the object Chapter~\ref{chap:couplings} studies, a
  ...
  of bijections of $\{0,1\}^{\mathcal{R}}$, one per ordered
  ...
  relabelling of the random variables that absorb the change of secrets. Since a
  bijection preserves the uniform law (Section~\ref{sec:prob-independence}),
  pushing the uniform randoms through it turns this pointwise
  ...
  properties that we care about, not any one procedure that realises them.
  ...
  Definition~\ref{def:substitution}, hence also a shear
  ...
  \]
  which is a bijection of $\{0,1\}^{\mathcal{R}}$. It is the ``rewrite recipe''
  ...
  bijections, so the family $\{\Phi_{\mathbf{a},\mathbf{b}}\}$
  is the right coupling of $\mathbf{w}$.
  \end{proof}
