...
\EndFunction
\Statex
\Function{SimpleRewrite}{$\mathbf{w}$}
  \State $\mathit{todo} \gets \{\, r \in \mathrm{vars}(\mathbf{w}) : r \text{ occurs once, additively}, r \notin \mathbf{w} \,\}$
  \State \emph{(sort $\mathit{todo}$ w.r.t. increasing multiplicative depth)}
  \State $T \gets \langle\rangle$
  \While{$\mathit{todo} \neq \langle\rangle$ \textbf{and not} \Call{SecretFree}{$\mathbf{w}$}}
    ...
    \State drop $r$
    \State \emph{(a random now matching the pattern above, $\notin \mathbf{w}$, may join $\mathit{todo}$)}
  \EndWhile
  \State \Return $(\Call{SecretFree}{\mathbf{w}}, T)$ \Comment{shears now recorded --- \S\ref{sec:coupling-view}}
\EndFunction
\Statex
\Function{GeneralRewrite}{$\mathbf{w}, \mathit{used}, \mathit{fuel}, \mathit{factored}, T$}
  ...
  \If{$r = \bot$}
    ...
    \State \Return $\Call{GeneralRewrite}{\mathrm{Factor}(\mathbf{w}), \mathit{used}, \mathit{fuel}, \mathit{true}, T}$ \Comment{lazy factoring (\S\ref{sec:factoring-cost})}
  \EndIf
...
% refer to the soundness proof
  ...
  is proved only in the linear/affine case \textcolor{orange}{(nerede proof?)}.
  ...
  A \textsc{Secure} verdict means $\overline{\mathbf{w}}$ is syntactically
...
\end{algorithm}
  ...
  a \textsc{Secure} verdict on $\langle (d, \mathcal{W}) \rangle$ certifies that every $d$-subset
...
\end{algorithm}
  ...
  \[
    ...
    \;=\; \overline{u}(\rho) .
  \]
  The left-hand side is $u$ evaluated at the relabelled randomness space under the secrets
  $\mathbf{a}$, so it is a bit value; the first equality is the
  substitution lemma, the second one comes from the admission condition. Both copies of
