...
Section~\ref{sec:d-probing}, is every probe (Definition~\ref{def:probe})
...
% We need a less poetic explanation in this part. Be precise and accurate on what these components do!
All three components use couplings. When Component~II reaches a batch of
...
bijective relabelling of the randomness space (a coupling function) that
...
(Corollary~\ref{thm:rewrite-soundness}); Components~II and III only utilize its
...
secret-independent. Throughout this chapter a wire is handled as the
$\mathbb{F}_2$ polynomial that defines it, its expression $\wexpr{w_i}$ is read off from
the circuit graph (Definition~\ref{def:wire-expr}); the engine only ever inspects
and rewrites that syntax, while the wire functions of
Definition~\ref{def:wire-function} are what the soundness proofs appeal to. The engine
works constructively, rather than reasoning about
...
secret-free in the sense of Notation~\ref{not:fv-vars}, so no secret variable
occurs in the expression of any $w_i$. Termination with a secret-free image
certifies $\mathbf{w}$.
Corollary~\ref{thm:rewrite-soundness} in Section~\ref{sec:coupling-view} makes precise
...
closes the section by viewing the whole derivation as an explicit, replayable
...
\Function{CheckProbe}{$\mathbf{w}$}
  \Require probe $\mathbf{w}$ (Definition~\ref{def:probe}), handled as the tuple of expressions $\wexpr{w_i}$
...
\Function{GeneralRewrite}{$\mathbf{w}, \mathit{used}, \mathit{fuel}, \mathit{factored}, T$}
  ...
  \State $r \gets \Call{FindSimple}{\mathbf{w}}$ \Comment{single-occurrence, additive, non-root}
...
\end{algorithm}
  ...
  \subsection{The Record \NoCaseChange{$T$}}
  \label{sec:record-T}

  A piece of data the algorithm keeps track of through every routine is $T$, so we
  clarify its contents. It is a list of pairs, initially empty and only ever
  appended to,
  \[
    T \;=\; \big\langle (r_1, t_1), \dots, (r_K, t_K) \big\rangle ,
    \qquad t_i \;=\; e_i \oplus r_i ,
  \]
  where the entry $(r_i, t_i)$ records that the engine replaced the random $r_i$ by
  the expression $t_i$, i.e.\ that it applied the substitution
  $\sigma_i = (r_i \mapsto t_i)$ of Definition~\ref{def:substitution}. The pairs
  appear in the order the steps were taken. Both rewriting routines push in the
  same format, \textproc{SimpleRewrite} pushes $(r, x)$ for the unique additive
  parent $x = e \oplus r$ it collapses, \textproc{GeneralRewrite} pushes $(r, t)$
  for the target $t = e \oplus r$ it substitutes.
  So $T$ is exactly a derivation in the sense of Definition~\ref{def:derivation}.
...
% refer to the soundness proof
  ...
  multi-occurrence random into a single-occurrence one; it also prefers the shallowest
  ...
  $x_1$ whose other children do not themselves depend on it, with the side condition
  ...
  where neither find-rule can see it, into an additive one.
  Factoring is symmetrically not tried before the
  ...
  retires its random into $\mathit{used}$ and it is never repeated, bounding the
  ...
  treated as failure, which is sound (Corollary~\ref{thm:rewrite-soundness} is never
  invoked on a fuel-out) but not necessarily tight. So, a fuel-out reports the probe as
  though it were genuinely insecure, without exhibiting an actual distinguishing
  observation.

  \subsection{The Factoring Step}
  \label{sec:factoring-step}

  \textproc{Factor} appears twice in Algorithm~\ref{alg:rewrite-engine}, as the
  second tier and again lazily inside the third, so we explain what
  it actually does.

  Both find-rules look for a random in \emph{additive} position, as an operand of a
  sum, because that is the only shape Lemma~\ref{lem:substitution} rewrites. A
  random sitting only inside products is therefore invisible to them, even when the
  probe could in fact be discharged. \textproc{Factor} reshapes the expression to
  expose such a random, by applying the distributive law right to left over a sum of
  products,
  \[
  ...
  \]
  pulling out a factor $f$ shared by two or more summands. The inner sum
  $a \oplus b$ is new additive structure, and a random trapped in it becomes
  reachable. Which factor to pull out is a heuristic choice, and the pass is run
  repeatedly and bottom-up; Section~\ref{sec:factoring} gives further implementation
  details.

  What matters here is that \textproc{Factor} is not a rewrite in the sense of
  Section~\ref{sec:randomness-substitution}. It substitutes nothing, retires no
  random, and pushes nothing onto $T$. The displayed equation is an identity over
  $\mathbb{F}_2$, so the expression changes while the function it denotes does not.
  Factoring only changes what the find-rules can see.

  \subsection{Soundness of the Rules}
  \label{sec:rule-soundness}

  We can now prove the soundess of Algorithm~\ref{alg:rewrite-engine} one
  routine at a time. Throughout, ``$T$ is a derivation
  for $\mathbf{w}$'' means in the sense of Definition~\ref{def:derivation}, as
  we have discussed in Section~\ref{sec:record-T}.

  \begin{lemma}[\textproc{Factor} preserves values]
  \label{lem:factor-sound}
  For every probe $\mathbf{w}$ and every secret assignment $\mathbf{a}$,
  $\textproc{Factor}(\mathbf{w})$ and $\mathbf{w}$ denote the same function:
  $\textproc{Factor}(\mathbf{w})(\rho; \mathbf{a}) = \mathbf{w}(\rho; \mathbf{a})$
  for every tape $\rho$.
  \end{lemma}

  % \begin{proof}
  %   This is trivial.
  % % Each application of the factoring identity replaces a subexpression by one with
  % % the same value, since distributivity holds in $\mathbb{F}_2$
  % % (Section~\ref{sec:field-f2}), and the denotation of an expression is built
  % % compositionally from its subexpressions
  % % (Section~\ref{sec:bool-circuits}). The pass is a finite number of such
  % % replacements.
  % \end{proof}
  This is clear.

  \begin{lemma}[\textproc{SimpleRewrite} is sound]
  \label{lem:simple-sound}
  If $\textproc{SimpleRewrite}(\mathbf{w}, \langle\rangle)$ returns $(\mathit{true}, T)$, then
  $T$ is a derivation for $\mathbf{w}$ that ends in a secret-free form.
  \end{lemma}

  \begin{proof}
  The loop pushes an entry only for a random $r$ drawn from $\mathit{todo}$, which
  by construction occurs exactly once in $\mathbf{w}$, additively, and is not a
  probe root. Its unique additive parent $x = e \oplus r$ satisfies
  $r \notin \mathrm{vars}(e)$, so $\sigma = (r \mapsto e \oplus r)$ is a
  substitution in the sense of Definition~\ref{def:substitution} and the step is an
  instance of Corollary~\ref{cor:single-occurrence-discharge}. Relabelling $x$ as a
  fresh random and dropping $r$ is simply the effect of applying $\sigma$. Each
  iteration therefore appends one legitimate substitution to the record. The loop
  returns $\mathit{true}$ only when \textproc{SecretFree}$(\mathbf{w})$ holds, that
  is when the current expression is secret-free.
  \end{proof}

  \begin{lemma}[\textproc{GeneralRewrite} is sound]
  \label{lem:general-sound}
  If a call to \textproc{GeneralRewrite} on $\mathbf{w}$ with record $T_0$ returns
  $(\mathit{true}, T)$, then $T$ extends $T_0$ to a derivation for $\mathbf{w}$
  that ends in a secret-free form.
  \end{lemma}

  \begin{proof}
  We do an induction on the recursion. The base case returns $\mathit{true}$ with
  $T = T_0$ only when $\mathbf{w}$ is already secret-free. A substitution step selects
  $r$ and an additive parent $x = e \oplus r$ with $r \notin \mathrm{vars}(e)$ (conditions
  both \textproc{FindSimple} and \textproc{FindGeneral} enforce)
  so $\sigma = (r \mapsto e \oplus r)$ is again a substitution in the sense of
  Definition~\ref{def:substitution}, and $\mathbf{w}[r \mapsto t]$ is its
  application (Lemma~\ref{lem:substitution}). The induction hypothesis applies to
  the recursive call with the record extended by $(r,t)$. A factoring step recurses
  on $\textproc{Factor}(\mathbf{w})$ with $T$ untouched, which is sound because
  Lemma~\ref{lem:factor-sound} leaves the denoted function unchanged, so a
  derivation for the factored form is a derivation for $\mathbf{w}$.
  \end{proof}

  % Nothing above depends on the search order, the fuel budget, or the heuristics that
  % choose which random to retire next. Those govern only whether a derivation is
  % found, never whether a found one is valid.

  \subsection{Relation to the Coupling-Construction View}
  \label{sec:coupling-view}

  A discharge does more than answer \Secure{}/\Insecure{}. Read the right way, it
  hands back the object Chapter~\ref{chap:couplings} studies: a coupling for the
  probe, in the sense of Definition~\ref{def:coupling-function}. All the work has
  been done there, and this subsection only has to observe that what the engine
  records is a derivation.

  Discharging a probe amounts to finding a derivation
  (Definition~\ref{def:derivation}), a finite sequence of substitutions
  $\sigma_1, \dots, \sigma_K$ after which no secret variable remains. It does not
  matter which of the three tiers proposed a given step, nor whether the random it
  retires occurred once or many times: each $\sigma_i$ is a substitution in the
  sense of Definition~\ref{def:substitution}, hence a shear $\widehat{\sigma_i}$
  (Lemma~\ref{lem:substitution}), and the coupling is assembled from these shears
  and from nothing else. That is why the engine can record every step in the same
  way, and why the construction of Section~\ref{sec:derivation-witness} never needs
  to know how an individual rewrite was found.

  \begin{corollary}[Soundness of discharge]
  \label{thm:rewrite-soundness}
  If \textup{\textproc{CheckProbe}}$(\mathbf{w})$
  (Algorithm~\ref{alg:rewrite-engine}) returns \textup{\Secure{}}, then
  $\mathbf{w}$ is secret-independent, and the recorded derivation $T$ realises a
  coupling $\Phi_{\mathbf{a},\mathbf{b}} = \varphi_{\mathbf{b}} \circ
  \varphi_{\mathbf{a}}^{-1}$ of $\mathbf{w}$.
  \end{corollary}

  \begin{proof}
  \textproc{CheckProbe} returns \Secure{} in three places, and each is covered.
  The first and third are a $\mathit{true}$ from \textproc{SimpleRewrite} and from
  \textproc{GeneralRewrite} on $\mathbf{w}$ itself, so
  Lemma~\ref{lem:simple-sound} and Lemma~\ref{lem:general-sound} make $T$ a
  derivation for $\mathbf{w}$ ending secret-free. The second is a $\mathit{true}$
  from \textproc{SimpleRewrite} on the factored probe $\mathbf{w}_f$, which gives a
  derivation for $\mathbf{w}_f$; by Lemma~\ref{lem:factor-sound} $\mathbf{w}_f$
  denotes the same function as $\mathbf{w}$, so it is a derivation for $\mathbf{w}$
  as well. In every case $T$ is a derivation for $\mathbf{w}$ that ends
  secret-free, and Theorem~\ref{thm:derivation-coupling} applies.
  \end{proof}

  % The engine's task is therefore only to \emph{search} for a derivation; that any
  % derivation it finds is a valid certificate was settled in
  % Chapter~\ref{chap:couplings}. Component~III
  % (Section~\ref{sec:extension}) takes the recorded $T$ and puts it to work on the
  % rest of the circuit.
...
in batches as large as possible rather than one at a time.
  ...
  size $\sum_j c_j$. Two vacaous worklists items are
  ...
  as its inner workings will be described in Section~\ref{sec:extension}.
...
\Function{CheckAll}{$wl = \langle (c_1, W_1), \dots, (c_m, W_m) \rangle$}
  ...
  \If{$v = \Insecure{}$} \Return \Insecure{} \EndIf
  ...
  \ForAll{$(i_1, \dots, i_m) \in \textstyle\prod_{j} \{0, \dots, c_j\}$ \textbf{with} $(i_j)_j \neq (c_j)_j$}
    ...
    \If{$r = \Insecure{}$} \Return \Insecure{} \EndIf
  \EndFor
  \State \Return \Secure{}
\EndFunction
\end{algorithmic}
\end{algorithm}
  ...
  particular every probe in $\llbracket wl \rrbracket$, meaning that the branch is secure and
  ...
  every $W_j$ at least $c_j$ times.
  % This single fact is what makes the split below both exhaustive and
  ...
  \]
  a \emph{disjoint} union. Each $c$-subset $S \in \binom{A \uplus B}{c}$ lies in exactly the block
  ...
  recursion we are strictly decreasing the number of probes we have to consider,
  ...
  of their ground-set sizes, under the multiset order. Consider the partition
  $W_j = A_j \uplus B_j$, so $|A_j| < |W_j|$ exactly when $B_j \neq \emptyset$ and
  $|B_j| < |W_j|$ exactly when $A_j \neq \emptyset$, hence a factor's ground set shrinks
  precisely when the cut is non-trivial, and it never grows.

  Two facts make sure that the cut is non-trivial on some factor. Skipping the all-safe cell is
  the first. Every cell the algorithm recurses on has $(i_j)_j \neq (c_j)_j$, so
  some factor $j^\ast$ has $i_{j^\ast} < c_{j^\ast}$, and in particular
  $c_{j^\ast} \geq 1$. That cell is also non-empty, so its factor
  $(c_{j^\ast} - i_{j^\ast},\, B_{j^\ast})$ has a positive count and a non-empty
  ground set, giving $B_{j^\ast} \neq \emptyset$. The representative is the second fact,
  because $c_{j^\ast} \geq 1$ it draws at least one wire from $W_{j^\ast}$, and it
  lies in $\mathsf{safe}$, so $A_{j^\ast} \supseteq \mathbf{w} \cap W_{j^\ast} \neq
  \emptyset$.

  Both parts of $W_{j^\ast}$ are therefore non-empty, hence both strictly smaller
  than $W_{j^\ast}$,
  and every other $W_j$ is replaced by ground sets of size at most $|W_j|$. The
  multiset strictly decreases, so the recursion is well founded.
  ...
  is secret-independent by marginalisation. If a degenerate worklist occurs,
  $\llbracket wl \rrbracket$ is empty or it is the empty probe, which is trivially secure.
  ...
  $\llbracket wl_{(i_j)} \rrbracket$ for a worklist strictly smaller in the multiset
  order above, hence secret-independent by the induction hypothesis. Every probe of $\llbracket wl \rrbracket$ lies in
  ...
  of $\mathcal{W}$ is secret-independent, i.e.\ the circuit is $d$-probing secure.
  \end{proof}
  ...
  I's incompleteness verbatim. An \Insecure{} verdict is
  ...
  complete on it, but in genreal Component I is not complete, so a spurious \Insecure{}
  can happen. Crucially such an \Insecure{} answer enters through Component
...
is a bijection of the randomness space, and suppose that for
every wire $u$ in a set $U$ the composite $u \circ \varphi$ is independent of the secrets.
Here $u$ is read as the wire function of Definition~\ref{def:wire-function}, so
$u \circ \varphi$ is the map $\rho \mapsto u(\varphi(\rho); \mathbf{a})$; when we
call it \emph{secret-free} below we mean the expression obtained by applying the
recorded substitutions to $\wexpr{u}$, and Lemma~\ref{lem:extend-exact} is what
makes that syntactic test agree with the semantic condition stated here.
...
does the law of any subset of $U$. A single bijection that blinds
...
mechanisms from the weakest to the strongest and adopt the last.
  ...
  reason why we take one further step, and view the derivation as a coupling function.
  ...
  final test with an exact one. Section~\ref{sec:coupling-view} already showed how to view the
  recorded derivation as a single bijection $\varphi$ of the
  randomness space. Coupling-driven
...
\Function{Extend}{$\mathbf{w}, \varphi, C$}
  ...
  \ForAll{$u \in C \setminus \mathbf{w}$}
    ...
    \EndIf
    \If{\Call{SecretFree}{$u \circ \varphi$} \textbf{ or } \Call{AnfFree}{$u \circ \varphi$}\footnote{\textproc{AnfFree}$(w)$ returns true exactly when no secret variable occurs in the expression of $w$ after it is taken to its ANF form.}} \Comment{Phases 2--3}
...
\end{algorithm}
  ...
  The fast filter is an evaluation under two runs. It fixes random values for the
  randoms, evaluates $u \circ \varphi$ once with the secrets also drawn at random and
  once with the secrets set to zero, and compares. If the two outcomes disagree, then $u \circ
  ...
  dependence less likely but never impossible, which is why the filter is
  allowed to reject but never to accept.

  Then the derivation is composed with $u$, and the
  ...
  bijection of the randomness space
