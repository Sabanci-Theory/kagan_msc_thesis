...
% \definecolor{hlrand}{HTML}{343d46}
...
\definecolor{hlrand}{HTML}{1F6FBE}    % randomness (blue)
...
\tikzset{
  ...
  randin/.style={wire, draw=hlrand, fill=hlrand!31},
  secin/.style={wire, draw=hlbad, fill=hlbad!30},
  randw/.style={wire, draw=hlrand, thick, fill=hlrand!10},
  probed/.style={draw=sari, very thick, fill=sari!30},
  safe/.style={draw=hlsafe, very thick, fill=hlsafe!12},
  subs/.style={draw=mor, very thick, fill=mor!18},
...
\begin{frame}{The story of this talk}
  \begin{enumerate}
    ...
    \item<2-> \textbf{The question:} was the masking done correcly? ``Correct''
...
\begin{frame}{Side channels, in the real world}
  This is not a theoretical worry. DPA has recovered keys from deployed
  smartcards, and timing side channels underlie the Spectre and Meltdown
  attacks against modern processors.
  \medskip
  \begin{center}
    \includegraphics[width=0.92\textwidth]{../Figures/spectre.png}
  \end{center}
\end{frame}
...
\begin{frame}{The probing model}
  ...
  \begin{block}{$d$-probing security}
    An adversary picks any $d$ wires (probe; observation set) $O = \{w_1, \dots, w_d\}$ and is handed
...
\begin{frame}{Counting is intractable}
  ...
  \begin{itemize}
    \item<2-> Hardness: reduction from \textsc{ExactSat}. One secret bit
      and one probed wire already encodes it.
...
\begin{frame}{What the placement tells}
  \begin{itemize}
    \item A polynomial time exact decision would collapse the \emph{entire} polynomial
      hierarchy to $\mathrm{P}$ \\ \hfill \begin{small}(Toda: $\mathrm{PH} \subseteq \mathrm{P}^{\sharpP}
      = \mathrm{P}^{\CeqP}$).\end{small}
    \item Deciding never costs more than counting, i.e., $\CeqP \subseteq \mathrm{P}^{\sharpP}$.
    \item A single probe is already in $\CeqP$, and full verification universally
    quantifies over exponentially many such instances.
  \end{itemize}
  \medskip
  \begin{exampleblock}{Consequence}
    We do not compute counts. We certify \emph{constructively}: exhibiting a
    relabelling of the randomness under which the secrets visibly cancel, giving
...
\begin{frame}{One substitution, on the running example}
  ...
  \begin{columns}[c]
    \begin{column}{0.5\textwidth}
      \centering
      \domand{wc01/.style={wire,probed}, wr/.style={wire,subs}}
    \end{column}
    \begin{column}{0.46\textwidth}
      ...
      \begin{itemize}
        ...
        \item<3-> And $\sigma$ induces a \emph{bijection} of the tape space,
          translating the coordinate $r$, a \emph{shear}.
        \item<4-> But, the context $x_0 \odot y_1$ mentions a share, so the
          \emph{one} syntactic step induces a \emph{family} of shears.
          Fixing the secrets $\va$ picks $\widehat{\sigma}^{\va}$.
      \end{itemize}
\end{column}
  \end{columns}
  \medskip
  \onslide<4->{Key identity: \;
  $(\mathbf{w}\sigma)(\rho;\va) = \mathbf{w}(\widehat{\sigma}^{\va}(\rho);\va)$,
  syntax and semantics agree, for every $\va$.}
\end{frame}
...
\begin{frame}{Security \emph{is} a coupling}
  ...
  \begin{itemize}
    ...
    \item ($\Rightarrow$) Equal laws mean the outcome-fibres
      ...
      \hfill {\small(full proof @ the appendix slides)}
  \end{itemize}
  \medskip
  \onslide<2->{
...
\begin{frame}{From substitutions to couplings}
  ...
  \begin{tikzpicture}[
    ...
    \node[exp] (nf) at (3.8,-1.6) {\alert{$\overline{O}(\,\cdot\,)$}};
  ...
  \begin{itemize}
    \item<2-> Every run is a re-labelled view of the \emph{same} secret-free
      experiment, so any two runs factor through it.
    ...
    \item<3-> So a derivation is a \textbf{reusable certificate}: the design
...
\begin{frame}{Pipeline overview}
  ...
  \medskip
  \begin{itemize}
    \item<3-> \textbf{I} decides a \emph{single} probe, by construction.
    \item<2-> \textbf{II} sweeps \emph{every} $d$-subset without explicitly enumerating
      them.
...
\begin{frame}{Component II: the representation}
  Listing all $\sum_{i \le d} \binom{|\Ws|}{i}$ probes is hopeless. So we never
  list them; we describe whole families compactly, as maskVerif does
  \parencite{10.1007/978-3-030-29959-0_15}.
  \medskip
  \begin{block}{Factors and worklists}
    \begin{itemize}
      \item \emph{factor} $(c, W)$ --- choose $c$ wires from $W$.
      \item \emph{worklist} $\langle (c_1, W_1), \dots, (c_m, W_m) \rangle$,
        the $W_j$ disjoint --- all probes taking $c_j$ from each $W_j$,
        $\prod_j \binom{|W_j|}{c_j}$ of them.
    \end{itemize}
  \end{block}
  \medskip
  \textbf{DOM-AND, order 2.} The eight computed wires
  $\Ws = \{u_0, u_1, p_{01}, p_{10}, c_{01}, c_{10}, s_0, s_1\}$, and the whole
  verification task is the single factor
  \[
    \bigl\langle (2, \Ws) \bigr\rangle
    \;=\; \text{all } \tbinom{8}{2} = 28 \text{ pairs, in one line.}
  \]
...
\begin{frame}{Component II: one recursion step, on our example}
  ...
  \begin{columns}[c]
    \begin{column}{0.44\textwidth}
      ...
      \only<1>{
        \domand{wu0/.style={wire,safe}, wc01/.style={wire,safe}}\\[0.5ex]
        {\scriptsize the illustrated probe set, in green}
      }
      ...
      \only<3>{
        \domand{wc01/.style={wire,probed}, ws1/.style={wire,probed}}\\[0.5ex]
        {\scriptsize a cross-term factor, in yellow}
      }

      \only<4>{
        \domand{wc10/.style={wire,probed}, wu1/.style={wire,probed}}\\[0.5ex]
        {\scriptsize an unsafe factor, in yellow}
      }
    \end{column}
    \begin{column}{0.52\textwidth}
      \begin{enumerate}
  ...
  \begin{columns}[c]
    ...
    \begin{column}{0.46\textwidth}
      \begin{itemize}
        ...
        \item<2-> Here $\varphi = (r \mapsto r \oplus x_0 \odot y_1)$ also
          blinds $u_0$ and $s_0$, as $u_0 \circ \varphi = x_0 \odot y_0$ and
          $s_0 \circ \varphi = x_0 \odot y_0 \oplus r$.
        \item<3-> Exact test via \emph{ANF}: a function depends on a variable
          iff it occurs in its normal form.
        \item<4-> One discharge settles the whole safe set; Component II
...
\begin{frame}{Implementation details}
  ...
  \begin{itemize}
    \item \textbf{Hash-consed DAG} expression representation with
      canonicalization, where structural sharing makes equality checks and
      substitutions cheap.
    ...
    \begin{column}{0.46\textwidth}
      \begin{itemize}
        ...
        \item<2-> Closures keep the verdicts but lose the yield: $14$ vs.\
          ...
          five-share ISW, because the split degrades toward enumeration.
...
\begin{frame}{Contributions}
  \begin{enumerate}
    \item<1-> \textbf{Placement.} We show that: counting is
      $\sharpP$-complete, deciding is $\CeqP$-complete, i.e., they sit above the
...
\begin{frame}{Backup: security is a coupling, full proof}
  \small
  Write $N_{\va}(o) = \bigl|\{\rho : O(\rho;\va) = o\}\bigr|$ for the tape counts.
  \medskip

  \textbf{($\Leftarrow$)} \; Let $\varphi$ be a coupling function from $\va$ to
  $\vb$. For any observation $o$, the identity $O(\rho;\va) = O(\varphi(\rho);\vb)$
  and the bijectivity of $\varphi$ give, reindexing by $\rho' = \varphi(\rho)$,
  \[
    N_{\va}(o)
    = \bigl|\{\rho : O(\rho;\va) = o\}\bigr|
    = \bigl|\{\rho : O(\varphi(\rho);\vb) = o\}\bigr|
    = \bigl|\{\rho' : O(\rho';\vb) = o\}\bigr|
    = N_{\vb}(o) .
  \]
  Hence $\law{\va}(O) = \law{\vb}(O)$; if this holds for every pair, $O$ is
  secret-independent by definition.
  \medskip

  \textbf{($\Rightarrow$)} \; Suppose $\law{\va}(O) = \law{\vb}(O)$, i.e.\
  $N_{\va}(o) = N_{\vb}(o)$ for every $o$. The fibres
  $F_{\va}(o) = \{\rho \mid O(\rho;\va) = o\}$ partition $\{0,1\}^{\Rs}$ as $o$
  ranges over the observations, and likewise the $F_{\vb}(o)$. Corresponding blocks
  have equal size, so pick a bijection $F_{\va}(o) \to F_{\vb}(o)$ for each $o$ and
  take their union $\varphi$. It is a bijection of $\{0,1\}^{\Rs}$, and by
  construction carries each $\rho \in F_{\va}(o)$ to some $\rho' \in F_{\vb}(o)$, so
  $O(\varphi(\rho);\vb) = o = O(\rho;\va)$. Thus $\varphi$ is a coupling function
  from $\va$ to $\vb$. \qed
\end{frame}
