...
\begin{frame}{Side-channel attacks}
  ...
  \begin{itemize}
    ...
    \item Differential power analysis [Kocher et al.~'99] correlates many power
...
\begin{frame}{Masking --- and our running example}
  \emph{Boolean masking} [Chari et al.~'99] splits each secret into shares,
  ...
  \begin{columns}[c]
    ...
    \begin{column}{0.44\textwidth}
      \textbf{DOM-AND} [Groß et al.~'16]: a masked AND gate, with two shares per input,
      and a fresh mask $r$.
      \medskip
      \begin{itemize}
...
\begin{frame}{The probing model}
  ...
  abstraction [Ishai--Sahai--Wagner~'03]:
    ...
    \begin{column}{0.46\textwidth}
      \begin{itemize}
        ...
        \item<3-> And thankfully, the abstraction is faithful: probing security implies
          noisy-leakage security [Duc--Dziembowski--Faust~'14]. We take
...
\begin{frame}{Warm-up: three views of secret-independence}
  ...
  \begin{enumerate}
    ...
    \item<2-> \textbf{Counting.} \; $N_{\va}(o) = N_{\vb}(o)$ for every
      ...
      \hfill {\small \emph{$\to$ this section}}
...
\begin{frame}{Counting is intractable}
  \begin{block}{Theorem (thesis, Ch.~4)}
    ...
    [Valiant~'79]. And deciding $\law{\va}(O) = \law{\vb}(O)$ is
    $\CeqP$-complete [Wagner~'86].
  \end{block}
  \medskip
  ...
  \onslide<2->{Observe that one secret bit and one probed wire already suffice; no feature
  of masking is what makes the problem hard.}
\end{frame}
...
\begin{frame}{Couplings}
  The classical idea [Barthe et al.~'17 for probabilistic programs] to prove that two
...
\begin{frame}{Component II: covering all probes}
  ...
  \end{block}
  \medskip
  \onslide<4->{The delicate point is that the split must \emph{lose nothing},
...
\begin{frame}{Component III: three mechanisms}
  \begin{itemize}
    ...
    \item<2-> \textbf{Replay} --- re-run the derivation on the candidate wire
      [after Barthe et al.~'15]. An imperfect fit for us because we \emph{factor},
...
\end{frame}
  ...
  \begin{itemize}
    ...
    \item<2-> \textbf{Four-share $x \oplus yz$ TI} [Bilgin et al.~'15]:
...
\begin{frame}{Backup: extension comparison (IV/IV)}
  ...
  quadratic classes that admit no uniform three-share TI [Bilgin et al.~'15].
\end{frame}
...
\begin{frame}{Backup: $\CeqP$ membership, sketch}
  ...
  and multiplication [Fenner et al.~'94]; the sum has $2^k \le 2^d$ terms,
  constant for fixed $d$. A sum of squares vanishes iff every term does, so
  \[
    \law{\va}(O) = \law{\vb}(O) \iff f = 0 ,
  \]
  and the problem is the vanishing set of a gap function: $\CeqP$.
\end{frame}

\begin{frame}{Backup: replay vs.\ factoring}
  Barthe et al.'s derivations replay two rule kinds: \textsc{Opt}
  (substitution) and \textsc{Conv} (ANF normalisation) --- for them, ANF is a
  \emph{replayed rule}, not a fallback.
  \medskip
...
\begin{frame}{Backup: positioning}
  \begin{itemize}
    \item \textbf{maskVerif} [Barthe et al.]: language-based, the traversal's
      main inspiration; replay-style extension; no factoring.
    \item \textbf{REBECCA / Fourier-analytic} [Bloem et al.]: correlation sets
      over Fourier coefficients; approximate in general.
    \item \textbf{SILVER} [Knichel et al.]: exact, BDD-based model counting;
      exhaustive per-probe distributions, different scaling profile.
    \item \toolname: exact like SILVER, constructive like maskVerif ---
...
\begin{frame}{Backup: the four-share TI netlist}
  Four-share TI of $x \oplus (y \odot z)$ [Bilgin et al.~'15]; each input
